 ...
 *
 * This version keeps the safer structure from your original server:
 * - credentials are loaded from environment variables only
 * - tools do not accept user/password/dsn parameters
 * - arbitrary SQL is read-only validated
 * - normal chat queries are row-limited
 * - large/full data requests are exported to files instead of returned to the LLM
 *
 * Required option A:
 *   ODBC_DSN
 *   ODBC_USER
...
const QUERY_TIMEOUT_MS = Number(process.env.QUERY_TIMEOUT_MS ?? 30 * 1000);

const ALLOW_FULL_EXPORT = String(process.env.ALLOW_FULL_EXPORT ?? "false").toLowerCase() === "true";
const FULL_EXPORT_MAX_ROWS = Number(process.env.FULL_EXPORT_MAX_ROWS ?? 100000);
const FULL_EXPORT_DIR = path.resolve(process.env.FULL_EXPORT_DIR ?? "./exports");

const MCP_SERVER_API_KEY = process.env.MCP_SERVER_API_KEY?.trim() || "";

const FORMAT_VALUES = ["json", "json_compact", "json_pretty", "jsonl", "md"];
const EXPORT_FORMAT_VALUES = ["csv", "jsonl", "json", "json_compact", "json_pretty", "md"];
...
function clampExportRows(value) {
  const requested = Number(value ?? FULL_EXPORT_MAX_ROWS);

  if (!Number.isFinite(requested) || requested <= 0) {
    return FULL_EXPORT_MAX_ROWS;
  }

  return Math.min(Math.floor(requested), FULL_EXPORT_MAX_ROWS);
}
...
async function runMetadataQuery(callback) {
  ...
  try {
    ...
    return await withTimeout(callback(connection), QUERY_TIMEOUT_MS, "Metadata query");
  } finally {
    if (connection) {
      try {
...
async function runReadonlyQueryRows(query, maxRows) {
  let connection;

  try {
    const safeQuery = validateReadonlyQuery(query);
    const limitedMaxRows = clampMaxRows(maxRows);
    const fetchRows = limitedMaxRows + 1;
    const limitedQuery = applySqlAnywhereLimit(safeQuery, fetchRows);

    connection = await connectDb();

    const startedAt = Date.now();
    const result = await withTimeout(connection.query(limitedQuery), QUERY_TIMEOUT_MS, "Read-only query");
    const durationMs = Date.now() - startedAt;

    const fetchedRows = sanitizeRows(toPlainRows(result));
    const rows = fetchedRows.slice(0, limitedMaxRows);

    return {
      rows,
      metadata: {
        returned_rows: rows.length,
        max_rows: limitedMaxRows,
        truncated: fetchedRows.length > rows.length,
        limited_at_database: true,
...
async function exportRowsToFile(rows, format, metadata, baseName = "query") {
  await fs.promises.mkdir(FULL_EXPORT_DIR, { recursive: true });

  const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
  const extension = format === "json_compact" || format === "json_pretty" ? "json" : format;
  const filename = `${baseName}-${timestamp}-${randomUUID().slice(0, 8)}.${extension}`;
  const filePath = path.join(FULL_EXPORT_DIR, filename);

  let content;

  if (format === "csv") {
    content = dataToCSV(rows);
  } else if (format === "jsonl") {
    content = rows.map((row) => JSON.stringify(row)).join("\n");
  } else if (format === "md") {
    content = dataToMD(rows);
  } else if (format === "json_pretty") {
    content = JSON.stringify({ metadata, rows }, null, 2);
  } else {
    content = JSON.stringify({ metadata, rows });
  }
...
async function exportReadonlyQuery(query, maxRows, format) {
  if (!ALLOW_FULL_EXPORT) {
    throw new Error("Full export is disabled. Set ALLOW_FULL_EXPORT=true to enable export tools.");
  }
  ...
  try {
    const safeQuery = validateReadonlyQuery(query);
    const exportMaxRows = clampExportRows(maxRows);
    const fetchRows = exportMaxRows + 1;
    const limitedQuery = applySqlAnywhereLimit(safeQuery, fetchRows);

    connection = await connectDb();

    const startedAt = Date.now();
    const result = await withTimeout(connection.query(limitedQuery), QUERY_TIMEOUT_MS, "Export query");
    const durationMs = Date.now() - startedAt;

    const fetchedRows = sanitizeRows(toPlainRows(result));
    const rows = fetchedRows.slice(0, exportMaxRows);
    const truncated = fetchedRows.length > rows.length;
    ...
    const metadata = {
      ...
      format,
      duration_ms: durationMs,
      note: "Rows were written to a file instead of returned to the LLM context.",
    };

    const fileInfo = await exportRowsToFile(rows, format, metadata, "query");
...
function createServer() {
  ...
  server.tool(
    ...
    "Return a compact summary of schemas, table/view counts, and sample tables.",
    {
  ...
  server.tool(
    ...
    async ({ q, schema = "%", max_results = 100, format = "json_compact" }) => {
      ...
      try {
        ...
        const response = successText(formatData(rows, format, { returned_rows: rows.length, max_results: maxResults }));
        setCache(cacheKey, response, METADATA_CACHE_TTL_MS);
        return response;
      } catch (error) {
        return errorResult(error);
  ...
  server.tool(
    ...
    async ({ schema, table, format = "json_compact" }) => {
      ...
      try {
        ...
        const response = successText(formatData(rows, format, { schema, table, column_count: rows.length }));
        setCache(cacheKey, response, METADATA_CACHE_TTL_MS);
        return response;
      } catch (error) {
        return errorResult(error);
        ...
        const rows = await runMetadataQuery(async (connection) => {
          ...
          if (typeof connection.primaryKeys === "function") {
            try {
              ...
              relationshipRows.push({ relationship_type: "primary_key_error", message: redactSecrets(error.message) });
            }
          } else {
            relationshipRows.push({
          ...
          if (typeof connection.foreignKeys === "function") {
            try {
              ...
              relationshipRows.push({ relationship_type: "foreign_key_error", message: redactSecrets(error.message) });
            }
          } else {
            relationshipRows.push({
  ...
  server.tool(
    "get_table_sample",
    "Return a safe row-limited sample from a table or view. Generated SQL only; no custom query required.",
    {
      schema: z.string(),
      table: z.string(),
    ...
    async ({ schema, table, columns = [], max_rows = DEFAULT_MAX_ROWS, format = "json_compact" }) => {
      try {
        ...
        const selectedColumns = Array.isArray(columns) && columns.length > 0
          ...
          : "*";
        const query = `SELECT TOP ${fetchRows} ${selectedColumns} FROM ${qualifiedTableName(schema, table)}`;

        let connection;
        try {
          connection = await connectDb();
          const startedAt = Date.now();
          const result = await withTimeout(connection.query(query), QUERY_TIMEOUT_MS, "Table sample query");
          const durationMs = Date.now() - startedAt;
          const fetchedRows = sanitizeRows(toPlainRows(result));
    ...
    async ({ schema, table, format = "json_compact" }) => {
      ...
      try {
        ...
        try {
          connection = await connectDb();
          const startedAt = Date.now();
          const result = await withTimeout(connection.query(query), QUERY_TIMEOUT_MS, "Table count query");
          const durationMs = Date.now() - startedAt;
          const rows = sanitizeRows(toPlainRows(result));
          const response = successText(formatData(rows, format, { duration_ms: durationMs }));
  ...
  server.tool(
    ...
    async ({ schema, table, column, max_rows = DEFAULT_MAX_ROWS, format = "json_compact" }) => {
      const cacheKey = makeCacheKey("get_distinct_values", { schema, table, column, max_rows, format });
      const cached = getCache(cacheKey);
      if (cached) return cached;
      ...
      try {
        ...
        try {
          connection = await connectDb();
          const startedAt = Date.now();
          const result = await withTimeout(connection.query(query), QUERY_TIMEOUT_MS, "Distinct values query");
          const durationMs = Date.now() - startedAt;
          const rows = sanitizeRows(toPlainRows(result));
          const response = successText(formatData(rows, format, { returned_rows: rows.length, max_rows: maxRows, duration_ms: durationMs }));
          setCache(cacheKey, response, QUERY_CACHE_TTL_MS);
          return response;
        } finally {
          if (connection) {
  ...
  server.tool(
    "run_readonly_query",
    "Run a read-only SELECT/WITH SQL query against the configured ODBC database. Results are row-limited for chat usage.",
    {
      query: z.string(),
      max_rows: z.number().int().positive().optional(),
      format: z.enum(FORMAT_VALUES).optional(),
    },
    async ({ query, max_rows = DEFAULT_MAX_ROWS, format = "json_compact" }) => {
      const safeForKey = normalizeSql(query);
      const cacheKey = makeCacheKey("run_readonly_query", { query: safeForKey, max_rows, format });
      const cached = getCache(cacheKey);
      if (cached) return cached;

      try {
        const { rows, metadata } = await runReadonlyQueryRows(query, max_rows);
  ...
  server.tool(
    "query_database_md",
    "Convenience alias for run_readonly_query with Markdown output. Still read-only and row-limited.",
    {
      query: z.string(),
      max_rows: z.number().int().positive().optional(),
    },
    async ({ query, max_rows = DEFAULT_MAX_ROWS }) => {
      try {
        const { rows, metadata } = await runReadonlyQueryRows(query, max_rows);
  ...
  server.tool(
    ...
    "Convenience alias for run_readonly_query with JSONL output. Still read-only and row-limited.",
    {
      query: z.string(),
      max_rows: z.number().int().positive().optional(),
    },
  ...
  server.tool(
    ...
    "Export a read-only SELECT/WITH query result to a local file. Rows are not returned to the LLM context.",
    {
      query: z.string(),
      max_rows: z.number().int().positive().optional(),
      format: z.enum(EXPORT_FORMAT_VALUES).optional(),
    },
    async ({ query, max_rows = FULL_EXPORT_MAX_ROWS, format = "csv" }) => {
      try {
        const result = await exportReadonlyQuery(query, max_rows, format);
  ...
  server.tool(
    ...
    async ({ schema, table, columns = [], max_rows = FULL_EXPORT_MAX_ROWS, format = "csv" }) => {
      try {
        const selectedColumns = Array.isArray(columns) && columns.length > 0
          ? columns.map(quoteIdentifier).join(", ")
          : "*";
        const query = `SELECT ${selectedColumns} FROM ${qualifiedTableName(schema, table)}`;
        const result = await exportReadonlyQuery(query, max_rows, format);
        return jsonText({ ...result, schema, table });
      } catch (error) {
        return errorResult(error);
      }
    }
...
app.get("/", (_req, res) => {
  res.json({
    ...
    defaults: {
      ...
      full_export_enabled: ALLOW_FULL_EXPORT,
      full_export_max_rows: FULL_EXPORT_MAX_ROWS,
      full_export_dir: FULL_EXPORT_DIR,
    },
  });
});
